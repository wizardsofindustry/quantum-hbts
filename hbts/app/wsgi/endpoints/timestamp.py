...
from ..schema import TimestampingRequest


class TimestampEndpoint(sq.interfaces.http.Endpoint):
    ...
    following functionality:
        ...
        Exposes an API to create and verify secure timestamps using an upstream Time Stamping Authority (TSA).


    A :class:`TimestampEndpoint` validates the structure of the request headers,
    URL parameters, query parameters and entity prior to forwarding the
    request to its handler (controller).

    The handler function (e.g., :meth:`~TimestampCtrl.get()`) may,
    instead of a :class:`~sq.interfaces.http.Response` object, return a tuple
    or a dictionary. The :class:`TimestampEndpoint` instance will interpret
